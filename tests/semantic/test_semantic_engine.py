...
def test_enrich_edges_with_semantics(semantic_engine):
    ...
    edges = [
        AdjacencyEdge(
            source_table="Sales", target_table="Customers",
            source_columns=("CustomerKey",), target_columns=("CustomerKey",),
            join_success_ratio=1.0, hybrid_score=0.0, evidence=""
        ),
        AdjacencyEdge(
            source_table="Returns", target_table="Products",
            source_columns=("TerritoryKey",), target_columns=("ProductKey",),
            join_success_ratio=1.0, hybrid_score=0.0, evidence=""
        ),
        AdjacencyEdge(
            source_table="Sales", target_table="DimA",
            source_columns=("Price",), target_columns=("CustomerName",),
            join_success_ratio=0.95, hybrid_score=0.0, evidence=""
        )
    ]
