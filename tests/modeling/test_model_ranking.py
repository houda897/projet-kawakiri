...
class TestModelRanking(unittest.TestCase):
    ...
    @patch('modeling.model_ranking.PARSIMONY_WEIGHTS', {
        "table_penalty": -1.0,
    ...
    def test_calculate_score_star_schema(self):
        ...
        candidate = DummyCandidate("star_1", DecisionModelType.STAR, ["fact_A"], ["dim_1", "dim_2"], 3, 10, 4)
        
        score = self.ranking_engine._calculate_score(candidate) # type: ignore
        self.assertEqual(score, 14.0)

    @patch('modeling.model_ranking.PARSIMONY_WEIGHTS', {
        "table_penalty": -1.0, "attribute_penalty": -0.1, "numeric_reward": 2.0,
        "dimension_reward": 5.0, "fact_coverage_bonus": 10.0, "shared_dimension_bonus": 2.0
    })
    def test_calculate_score_constellation(self):
        """Test the score with the bonuses of a constellation model"""
        candidate = DummyCandidate("const_1", DecisionModelType.CONSTELLATION, ["fact_A", "fact_B"], ["dim_1", "dim_2"], 4, 20, 5)
    ...
    def test_rank_and_store(self, mock_clear):
        ...
        cand2 = DummyCandidate("model_2", DecisionModelType.STAR, ["F2"], ["D1", "D2"], 3, 5, 4)
