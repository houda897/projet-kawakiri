...
def dummy_columns():
    ...
    return [
        PrimaryKeyCandidate(
            database_name="TestDB", table_name="FactSales",
            column_name="OrderNumber", column_type="String",
            rows=1000, null_ratio=0.0, uniqueness_ratio=0.5,
            identifiability_score=0.8, confidence=0.9, reason="test"
        ),
        PrimaryKeyCandidate(
            database_name="TestDB", table_name="FactSales",
            column_name="ProductKey", column_type="Int32",
            rows=1000, null_ratio=0.0, uniqueness_ratio=0.1,
            identifiability_score=0.9, confidence=0.8, reason="test"
        ),
        PrimaryKeyCandidate(
            database_name="TestDB", table_name="FactSales",
            column_name="OrderDate", column_type="Date",
            rows=1000, null_ratio=0.0, uniqueness_ratio=0.05,
            identifiability_score=0.4, confidence=0.4, reason="test"
        )
    ]
...
def test_generate_composite_candidates_no_columns(composite_engine):
    ...
    composite_engine.load_columns_for_composite_search = MagicMock(return_value=[
        PrimaryKeyCandidate(
            database_name="TestDB", table_name="TinyTable",
            column_name="OnlyCol", column_type="String",
            rows=10, null_ratio=0.0, uniqueness_ratio=1.0,
